...
import argparse
import numpy as np
from model.capsule_resnet import ResNet18
...
arch = 'cifar10_capsule'
filename = arch +'_'+str(num_class)
checkpoint_filename = os.path.join(args.checkpoint, filename+'.pt')


net = ResNet18(num_units=16, num_class=num_class, image_channels=image_channels)
...
def to_one_hot(x, length=num_class):
    batch_size = x.size(0)
    x_one_hot = torch.zeros(batch_size, length)
    for i in range(batch_size):
        x_one_hot[i, x[i]] = 1.0
    return x_one_hot.cuda()
